...
(function ($) {
	$(document).ready(function () {
		...
		$('li', tabcontainer).eq(0).addClass('active').next().slideDown(300);
		$('.tab-title').click(function (event) {
			if ($(this).hasClass('active')) {
				$(this).removeClass('active').next().stop(true, false).slideUp(300);
			} else {
				$('.tab-title', tabcontainer).removeClass('active');
				$('.tab-content').slideUp(300);
				$(this).addClass('active').next().stop(true, false).slideDown(300);
			}
		});
...
$inputTo.on("input", function () {
    ...
    instance.update({
		...
		function scroll_element() {
			...
			if ($top >= 400 && !($('body').hasClass('menuStick'))) {
				$('body').addClass('menuStick');
			} else if ($top < 200 && $('body').hasClass('menuStick')) {
				$('body').removeClass('menuStick');
			}
		}
		function scroll_elementx() {
			/*Add class menuStick to body*/
			$tops = $(window).scrollTop();
			if ($tops >= 200 && !($('body').hasClass('menuLeft'))) {
				$('body').addClass('menuLeft');
			} else if ($tops < 200 && $('body').hasClass('menuLeft')) {
		...
		$('.tabs-nav a').on('click', function (event) {
			...
			$($(this).attr('href')).show();
				...
				// Show the first tab by default
				$('.hide-giaodien').hide();
				$('.tabs-strong div:first').show();

				$('.khogiaodien-slider a').on('click', function (event) {
					event.preventDefault();
					$('.tabs-nav li').removeClass('tab-active');
					$(this).parent().addClass('tab-active');
					$('.hide-giaodien').hide();
		...
		// Show the first tab by default
		$('.tabs-navs li:first').addClass('tab-active');

		// Change tab class and display content
		$('.tabs-navs a').on('click', function (event) {
			$('.tabs-navs li').removeClass('tab-active');
			$(this).parent().addClass('tab-active');
			$('.tabs-stage div').hide();
			$($(this).attr('href')).show();
		});
		...
		$('.custom-slider-box').owlCarousel({
			animateOut: 'fadeOut',
			animateIn: 'fadeIn',
			slideSpeed: 3000,//Tốc Độ Slider
			items: 1,//SỐ Slide hiển thị
			mouseDrag: false,//Vô Hiệu Hóa Chuột
			touchDrag: false,//
			center: true,
		...
		$('.khogiaodien-slider').owlCarousel({
			slideSpeed: 3000,//Tốc Độ Slider
			items: 7,//SỐ Slide hiển thị
			center: true,
			responsiveClass:true,
			loop: true,
			nav:true,
		...
		var swiper4 = new Swiper('.co-nuoc .swiper-container', {
			slidesPerView: 8,
			paginationClickable: true,
			nextButton: '.co-nuoc .swiper-btn-next',
			prevButton: '.co-nuoc .swiper-btn-prev',
			spaceBetween: 15,
			paginationClickable: true,
			autoplay: 2000,
			loop: true,
